        ...
        $.ajax({
            ...
            success: function(response){
                ...
                for (j in response) {
                    ...
                    $.ajax({
                        method: 'GET',
                        url: 'http://0.0.0.0:5001/api/v1/places/' + p.id + '/reviews',
                        headers: {
                            'Content-Type': 'application/json'
                        }
                    }).done(function(rData){
                        let result = '';
                        var ln = Object.keys(rData).length;
                        for (let rv in rData) {
                            let vl = rData[rv];
                            result += vl.text + '<br><br>';
                       ...
                       $('.places').append('<article><div class="title_box"><h2>' + p.name + '</h2><div class="price_by_night">$' + p.price_by_night + '</div></div>\
                        ...
                        </div><div class="user"><b>Owner:</b></div><div class="description">' + p.description + '</div>\
                        <div class="reviews"><h2>' + ln + ' Reviews<h1><span id="sh">show</span></h1></h2>' + result + '</div></article>');
